...
func (msg *MsgCreateUserVault) ValidateBasic() error {
	...
	if msg.Balance == 0 {
		return sdkerrors.Wrapf(ErrZeroTokens, "invalid balance (%d)", msg.Balance)
	}
...
func (msg *MsgUpdateUserVault) ValidateBasic() error {
	...
	if msg.Balance == 0 {
		return sdkerrors.Wrapf(ErrZeroTokens, "invalid balance (%d), try delete instead", msg.Balance)
	}
